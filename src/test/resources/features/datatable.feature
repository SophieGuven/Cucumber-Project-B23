Feature: Demonstrating the data table feature of cucumber
  ...
  using the data table feature and automatically convert the
  table into few known dadta types supported by cucumber
...
@ui
  ...
  Scenario: Representing table with header and more than 2 columns with list of map
    #we want to represent each row as a map obj with column name as key value as cell value
